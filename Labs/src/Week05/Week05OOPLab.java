...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		Card1 card = new Card1("Two", "Hearts", 2);
		card.describe();
		...
		Map<String, List<Card1>> gameBoard = new HashMap<>();
		...
		for (int i = 1; i <= numberOfPlayers; i++) {
			...
			List<Card1> playerList = gameBoard.get(playerName);
			for (Card1 handCard : playerList) {
				handCard.describe();
			}
			System.out.println("-------------------");
		}
	...
	public static Map<String, List<Card1>> createGame (int numberOfPlayers){
		Map<String, List<Card1>> finalGame = new HashMap<>();
		Deck1 deck = new Deck1();
		deck.shuffle();
		System.out.println("Deck has " + deck.getCards().size()+ "cards");
		
		for (int i = 1; i <= numberOfPlayers; i++) {
			List<Card1> playerList = new ArrayList<>();
			String playerName = "Player " + i;
			finalGame.put(playerName, playerList);
		}
		
		for (int i = 0; i < 52/numberOfPlayers; i++) {
			for (int j = 1; j <= numberOfPlayers; j++) {
				String playerName = "Player " + j;
				List<Card1> playerList = finalGame.get(playerName);
				playerList.add(deck.draw());
				finalGame.replace(playerName, playerList);
			}
		}
		return finalGame;
	}
